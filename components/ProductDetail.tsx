...
const ProductDetail: React.FC<ProductDetailProps> = ({ product, onBack, onAddToCart }) => {
  ...
  const usdPrice = Math.round(product.price / 130);
      ...
      <div className="max-w-[1800px] mx-auto px-6 md:px-12 pb-24">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-24">
          ...
          <div className="flex flex-col justify-center max-w-xl">
             ...
             <div className="mb-8 group cursor-help inline-block w-fit">
                <span className="text-2xl font-light text-[#2C2A26] group-hover:hidden">KSh {product.price.toLocaleString()}</span>
                <span className="text-2xl font-light text-[#2C2A26] hidden group-hover:inline">${usdPrice}</span>
             </div>
             ...
             <div className="flex flex-col gap-4">
               <button 
                 ...
                 className="w-full py-5 bg-[#2C2A26] text-[#F5F2EB] uppercase tracking-widest text-sm font-medium hover:bg-[#433E38] transition-colors group"
               >
                 Add to Cart — <span className="group-hover:hidden">KSh {product.price.toLocaleString()}</span><span className="hidden group-hover:inline">${usdPrice}</span>
               </button>

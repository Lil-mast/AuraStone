          ...
          <div className="lg:pl-12 lg:border-l border-[#D6D1C7]">
               ...
               {items.map((item, idx) => (
                 <div key={idx} className="flex gap-4">
                    ...
                    <div className="group relative">
                        <span className="text-sm text-[#5D5A53] group-hover:hidden">KSh {item.price.toLocaleString()}</span>
                        <span className="text-sm text-[#5D5A53] hidden group-hover:block">${Math.round(item.price / 130)}</span>
                    </div>
                 </div>
               ))}
            </div>
            ...
            <div className="border-t border-[#D6D1C7] pt-6 space-y-2">
              <div className="flex justify-between text-sm text-[#5D5A53]">
                 <span>Subtotal</span>
                 <div className="group">
                    <span className="group-hover:hidden">KSh {subtotal.toLocaleString()}</span>
                    <span className="hidden group-hover:block">${Math.round(subtotal / 130)}</span>
                 </div>
              </div>
              <div className="flex justify-between text-sm text-[#5D5A53]">
                 <span>Shipping</span>
            ...
            <div className="border-t border-[#D6D1C7] mt-6 pt-6">
               <div className="flex justify-between items-center">
                 ...
                 <div className="flex items-end gap-2 group">
                   <span className="text-xs text-[#A8A29E] mb-1 group-hover:hidden">KES</span>
                   <span className="font-serif text-2xl text-[#2C2A26] group-hover:hidden">KSh {total.toLocaleString()}</span>
                   
                   <span className="text-xs text-[#A8A29E] mb-1 hidden group-hover:inline">USD</span>
                   <span className="font-serif text-2xl text-[#2C2A26] hidden group-hover:inline">${Math.round(total / 130)}</span>
                 </div>
               </div>
            </div>
          </div>
